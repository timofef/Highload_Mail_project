...
Для хранения файлов аттачей воспользуемся [MinIO](https://min.io/), который работает по протоколу S3. Вложения будут храниться по пути /{author_id}/{attach_id}.
...
3. letter
    ...
    В среднем, почтовый ящик состоит из 8000 писем. Тогда письма всех пользователей без учета вложений в худшем случае будут занимать:
    ```
    Письма всех пользователей без учета вложений: 1000214 * (50 * 10^6) * 8000 ~ 4 * 10^17 байт
    ```
...
Будем использовать микросервисную архитектуру для обеспечения просто масштабируемости отдельных компонентов.

1. API - 
2. Auth - сервис авторизации. Отвечает за авторизацию, проверку, авторизован ли пользователь и определение id пользователя по ключу сессии.
3. User - отвечает за регистрацию и получение данных о пользователе.
4. Letters - отвечает за полчение писем из БД, чтение писем из БД
5. 
